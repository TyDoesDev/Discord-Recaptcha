...
const loadClientEvents = async function (client) {
    ...
    for (const folder of event_folders) {
        ...
        for (const file of event_files) {
            ...
            if (event.name)
                log(`Loaded event: ${event.name} successfully`, {
                    header: 'CLIENT_EVENT_LOADER',
                    type: 'ready'
                })
            else
                return log(`ERROR: Event: ${file} is missing a name or the name is not a string.`, {
                    header: 'CLIENT_EVENT_LOADER',
                    type: 'ready'
                })
...
const loadSlashCommands = async function (client) {
    ...
    for (const folder of command_folders) {
        ...
        for (const file of command_files) {
            ...
            if (command.name) {
                ...
                return log(
                    `Error loading command: ${file}, it most likely has a missing name or the name is not a string`,
                    {
                        header: 'CLIENT_COMMAND_LOADER',
                        type: 'error'
                    }
    ...
    client.on('ready', async () => {
        client.application.commands
            ...
            .then(() =>
                log('Slash commands have been registered with the discord api', {
            ...
            .catch(e =>
                log(`Failed to register slash commands: ${e.stack}`, {
                    header: 'DISCORD_APPLICATION_REGISTRY',
                    type: 'error'
                })
            )
